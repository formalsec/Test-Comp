...
void reach_error()
{
  __assert_fail("0", "float_req_bl_1231.c", 3, "reach_error");
}
...
int main()
{
  ...
  if (__logor(__logand(isnan_float(x), !isnan_float(y)), __logand(!isnan_float(x), isnan_float(y))))
  {
    float res = fmin_float(x, y);
    if (__logand(__logand(isnan_float(x), !isnan_float(y)), res != y))
    {
      {
        reach_error();
      }
    ...
    if (__logand(__logand(!isnan_float(x), isnan_float(y)), res != x))
    {
      {
        reach_error();
      }
