...
{
  char *dest = fullname;
  if (__logand(directory[0] == '/', !directory[1]))
  {
    *dest = '/';
    ++dest;
  }
  else
    if (__logor(directory[0] != '.', directory[1]))
  {
...
int main()
{
  int dir_len = __VERIFIER_nondet_int("dir_len");
  if (dir_len < 1)
  {
    return 1;
  }

  int file_len = __VERIFIER_nondet_int("file_len");
  if (file_len < 1)
  {
    return 1;
  }
