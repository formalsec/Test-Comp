...
void __CS_cs(void)
{
	...
	__CS_round += k;
	assume_abort_if_not(__CS_thread_lockedon[k][__CS_thread_index] == 0); // this is removed when not needed
...
{
	__CS_pthread_t t1 = __VERIFIER_nondet_uchar("t1");
	__CS_pthread_t t2 = __VERIFIER_nondet_uchar("t2");
	__CS_cs(); if (__CS_ret != 0) return 0;
	num[__CS_round] = 1;
	__CS_cs(); if (__CS_ret != 0) return 0;
...
int main()
{
	...
	for(i = 0; i < 2; i++) {
	  __CS_cp_num[i] = __VERIFIER_nondet_int("__CS_cp_num[i]");
	  __CS_cp_m[i] = __VERIFIER_nondet_uchar("__CS_cp_m[i]");
	  __CS_cp_empty[i] = __VERIFIER_nondet_uchar("__CS_cp_empty[i]");
	  __CS_cp_full[i] = __VERIFIER_nondet_uchar("__CS_cp_full[i]");
	  for(j = 0; j < 3; j++) {
	...
	assume_abort_if_not(empty[0] == __CS_cp_empty[1]);  //cseq: Consistency of empty
