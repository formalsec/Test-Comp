...
void __CS_cs(void)
{
  ...
  __CS_round += k;
  __CS_ret = (__logand(__VERIFIER_nondet_int("1"), __CS_round == (6 - 1))) ? (__CS_ret_PREEMPTED) : (__CS_ret);
}
...
void *main_thread(void *arg)
{
  ...
  if (__logor(i[__CS_round] >= 144, j[__CS_round] >= 144))
  {
    __CS_cs();
    if (__CS_ret != 0)
      return 0;
...
int main(void)
{
  ...
  for (int i = 0; i < 6; ++i)
  {
    ...
    __CS_cp_i[i] = __VERIFIER_nondet_int("__CS_cp_i[i]");
    __CS_cp_j[i] = __VERIFIER_nondet_int("__CS_cp_j[i]");
  }
