...
void reach_error()
{
  __assert_fail("0", "sqrt_Householder_interval.c", 3, "reach_error");
}
...
int main()
{
  double d;
  double r;
  d = __VERIFIER_nondet_double("d");
  assume_abort_if_not(__logand(d >= 0.1, d <= 20.0));
  r = SqrtR(d);
  __VERIFIER_assert(__logand(r >= 0.3, r <= 5.));
  return 0;
}
