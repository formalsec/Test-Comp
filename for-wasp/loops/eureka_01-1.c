...
void reach_error()
{
  __assert_fail("0", "eureka_01-1.c", 7, "reach_error");
}
...
int main()
{
  int nodecount = __VERIFIER_nondet_int("nodecount");
  int edgecount = __VERIFIER_nondet_int("edgecount");
  if (!__logand(0 <= nodecount, nodecount <= 4))
    return 0;

  if (!__logand(0 <= edgecount, edgecount <= 19))
    return 0;
