...
void reach_error()
{
  __assert_fail("0", "double_req_bl_1130a.c", 3, "reach_error");
}
...
double fmod_double(double x, double y)
{
  ...
  hx ^= sx;
  hy &= 0x7fffffff;
  if (__logor(__logor((hy | ly) == 0, hx >= 0x7ff00000), (hy | ((ly | (-ly)) >> 31)) > 0x7ff00000))
    return (x * y) / (x * y);

  if (hx <= hy)
  {
    if (__logor(hx < hy, lx < ly))
      return x;
